...
try {
    ...
    $scopeRaw = isset($_GET['scope']) ? trim((string) $_GET['scope']) : 'direct';
    if (!in_array($scopeRaw, ['all', 'direct', 'collaborators'], true)) {
        http_response_code(422);
        echo json_encode(['ok' => false, 'error' => 'scope debe ser all, direct o collaborators']);
        exit;
    }

    $service = new TeamHealthService(new TeamHealthRepository($pdo));
    $data = $service->computeForTeam($teamId, [
