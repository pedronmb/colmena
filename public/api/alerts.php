...
try {
    ...
    if ($method === 'PUT') {
        ...
        if ($teamId < 1 || $alertId < 1 || $title === '') {
            http_response_code(422);
            echo json_encode(['ok' => false, 'error' => 'team_id, alert_id y title son obligatorios']);
            exit;
        }
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $dueDate)) {
            http_response_code(422);
        ...
        $ok = $repo->update($alertId, $teamId, $title, $body, $dueDate);
        if (!$ok) {
        ...
        echo json_encode(['ok' => true, 'alert' => $updated], JSON_UNESCAPED_UNICODE);
        exit;
    }
